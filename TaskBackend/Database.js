    ...
    if (err) {
        ...
        db.run(`CREATE TABLE articles (
            ...
            if (err) {
                ...
                const INSERT_ARTICLE = 'INSERT INTO articles (title, author, article_text, date_published, date_edited, created_by) VALUES (?,?,?,?,?,?)';
                dummyData.forEach((data) => {
                    db.run(
                        INSERT_ARTICLE,
                        [data.title, 'Dummy Author', data.task_text, data.date_published, data.date_edited, data.created_by],
                        (err) => {
                            if (err) {
                                console.log("Error inserting dummy data into the articles table: " + err.message);
                            } else {
                                console.log("Dummy data inserted into the articles table.");
                            }
                        }
                    );
                });
